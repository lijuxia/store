...
public class WarehouseRecordServiceImpl implements WarehouseRecordService {
    ...
    public void insert(WarehouseRecord warehouseRecord){
        ...
        if(warehouseRecord.getType()==WarehouseRecord.TYPE_MAKE){//生产，半成品入库
            warehouseService.updateSave(WarehouseServiceImpl.OP_INTO,warehouseRecord.getStoreId(),warehouseRecord.getMakeProductId(),warehouseRecord.getMakeNum(),warehouseRecord.getDateTime());//半成品入库
        }
        for(int i = 0;i<details.size();i++){
            ...
            if(detail.getNum().compareTo(BigDecimal.ZERO)!=0){
                ...
                if(warehouseRecord.getType()==WarehouseRecord.TYPE_BUY){//进货
                    BigDecimal beforeSaveNum = warehouseService.updateSave(WarehouseServiceImpl.OP_INTO,warehouseRecord.getStoreId(),detail.getProductId(),detail.getNum(),warehouseRecord.getDateTime());
                    ...
                    BigDecimal beforeSaveNum = warehouseService.updateSave(WarehouseServiceImpl.OP_OUT,warehouseRecord.getStoreId(),detail.getProductId(),detail.getNum(),warehouseRecord.getDateTime());
                    detail.setBeforeSaveNum(beforeSaveNum);
                    ...
                    for(int p=0;p<product.getDetails().size();p++){
                        warehouseService.updateSave(WarehouseServiceImpl.OP_OUT,warehouseRecord.getStoreId(),product.getDetails().get(p).getDetailId(),detail.getNum().multiply(product.getDetails().get(p).getNum()),warehouseRecord.getDateTime());
                    }
                }else if(warehouseRecord.getType()==WarehouseRecord.TYPE_MAKE){//生产
                    BigDecimal beforeSaveNum = warehouseService.updateSave(WarehouseServiceImpl.OP_OUT,warehouseRecord.getStoreId(),detail.getProductId(),detail.getNum(),warehouseRecord.getDateTime());//原料出库
                    detail.setBeforeSaveNum(beforeSaveNum);
                    ...
                    if(detail.getNum().compareTo(BigDecimal.ZERO)!=-1){//大于等于0,可以修改库存,小于0不能修改库存,认为是无效数据
                        BigDecimal beforeSaveNum = warehouseService.updateSave(WarehouseServiceImpl.OP_CHANGE,warehouseRecord.getStoreId(),detail.getProductId(),detail.getNum(),warehouseRecord.getDateTime());//原料出库
                        detail.setBeforeSaveNum(beforeSaveNum);
                    }
                }
    ...
    public void confirmeRecord(String oddId){
        ...
        if(record!=null && record.getConfirmFlag()==WarehouseRecord.CONFIRMFLAG_NO){
            ...
            for(int i = 0;i<details.size();i++){
                WarehouseRecordDetail detail = details.get(i);
                //配送单位库存减少
                warehouseService.updateSave(WarehouseServiceImpl.OP_OUT,record.getStoreId(),detail.getProductId(),detail.getNum(),time);
                //接收单位库存增加
                warehouseService.updateSave(WarehouseServiceImpl.OP_INTO,record.getSendStoreId(),detail.getProductId(),detail.getNum(),time);
            }
        }
    }
    ...
    public void delete(String oddId){
        ...
        if(warehouseRecordOld!=null && warehouseRecordOld.getStatus()==WarehouseRecord.STATUS_ON){
            ...
            if(warehouseRecordOld.getType()==WarehouseRecord.TYPE_MAKE){//生产，半成品入库回退
                warehouseService.updateSave(WarehouseServiceImpl.OP_OUT,warehouseRecordOld.getStoreId(),warehouseRecordOld.getMakeProductId(),warehouseRecordOld.getMakeNum(),updateTime);//半成品入库
            }
            for(int i = 0;i<details.size();i++){
                WarehouseRecordDetail detail = details.get(i);
                if(detail.getNum().compareTo(BigDecimal.ZERO)!=0){
                    ...
                    if(warehouseRecordOld.getType()==WarehouseRecord.TYPE_BUY){//进货回退
                        warehouseService.updateSave(WarehouseServiceImpl.OP_OUT,warehouseRecordOld.getStoreId(),detail.getProductId(),detail.getNum(),updateTime);
                        ...
                        warehouseService.updateSave(WarehouseServiceImpl.OP_INTO,warehouseRecordOld.getStoreId(),detail.getProductId(),detail.getNum(),updateTime);
                    }else if(warehouseRecordOld.getType()==WarehouseRecord.TYPE_SALE){//销售、实际库存改变转换为原料改变,回退
                        Product product = productService.findById(detail.getProductId());
                        for(int p=0;p<product.getDetails().size();p++){
                            warehouseService.updateSave(WarehouseServiceImpl.OP_INTO,warehouseRecordOld.getStoreId(),product.getDetails().get(p).getDetailId(),detail.getNum().multiply(product.getDetails().get(p).getNum()),updateTime);
                        }
                    }else if(warehouseRecordOld.getType()==WarehouseRecord.TYPE_MAKE){//生产 回退
                        warehouseService.updateSave(WarehouseServiceImpl.OP_INTO,warehouseRecordOld.getStoreId(),detail.getProductId(),detail.getNum(),updateTime);//原料出库
                        ...
                        warehouseService.updateSave(WarehouseServiceImpl.OP_INTO,warehouseRecordOld.getStoreId(),detail.getProductId(),detail.getNum(),updateTime);
                        //接收单位库存回退减少
                        warehouseService.updateSave(WarehouseServiceImpl.OP_OUT,warehouseRecordOld.getSendStoreId(),detail.getProductId(),detail.getNum(),updateTime);
                        ...
                        warehouseService.updateSave(WarehouseServiceImpl.OP_INTO,warehouseRecordOld.getStoreId(),detail.getProductId(),num,updateTime);
                    }
                }else{
